  ...
  },
  {
    ...
    slides: [
      ...
      },
      {
        ...
        description: "Use AI to generate SQL for your ChatGPT wrapper database",
        task: "In Cursor, use this prompt: 'Help me create two tables for a ChatGPT wrapper: 1) conversations table with id (uuid, primary key), user_id (uuid, foreign key to auth.users), title (text, the chat title), created_at (timestamp). 2) messages table with id (uuid, primary key), conversation_id (uuid, foreign key to conversations), role (text: user or assistant), content (text, the message), created_at (timestamp). Write the SQL for me.'",
        ...
        verificationSteps: [
          ...
          "conversations has user_id, title, created_at",
          "messages has conversation_id, role, content, created_at",
          "Foreign keys properly set up",
        ],
        tips: [
          "Just copy the prompt exactly into Cursor",
          "Let AI write the SQL for you",
          "You'll paste this into Supabase next",
        ],
      },
      {
        type: "text",
  ...
  },
  {
    ...
    slides: [
      ...
      },
      {
        ...
        description: "Use AI to create functions for saving messages",
        task: "In Cursor, use this prompt: 'Create two functions: 1) createConversation(userId, title) - creates a new conversation and returns the conversation id. 2) saveMessage(conversationId, role, content) - saves a message (user or assistant) to the messages table. Use the Supabase client from utils/supabase/client.ts. Handle errors.'",
        expectedOutcome: "Functions that save conversations and messages",
        verificationSteps: [
          "createConversation function created",
          "saveMessage function created",
          "Uses Supabase client",
          "Handles errors",
          "Returns conversation/message IDs",
        ],
        tips: [
          "Copy the prompt exactly",
          "Let AI generate the insert code",
          "Test by creating a conversation and saving messages",
        ],
      },
      {
        type: "text",
  ...
  },
  {
    ...
    slides: [
      ...
      },
      {
        ...
        description: "Use AI to create functions for loading chats",
        task: "In Cursor, use this prompt: 'Create two functions: 1) getConversations(userId) - fetches all conversations for a user, ordered by created_at descending. 2) getMessages(conversationId) - fetches all messages for a conversation, ordered by created_at ascending. Use the Supabase client. Handle errors.'",
        expectedOutcome: "Functions that load conversations and messages",
        verificationSteps: [
          "getConversations function created",
          "getMessages function created",
          "Uses Supabase client",
          "Proper ordering (newest conversations first, oldest messages first)",
          "Handles errors",
        ],
        tips: [
          "Copy the prompt exactly",
          "Let AI generate the select queries",
          "Test by loading conversations and messages",
        ],
      },
      {
        type: "text",
